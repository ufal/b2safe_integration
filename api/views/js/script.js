...
function displayItem(item, container) {
    ...
    if(item.splitted) {
        if (item.replication_error) {
            ...
            repData = "<div><div><strong style=\"font-size: 12px;\">Uploaded in parts</strong></div>" +
            		"<table class=\"table table-sm\" style=\"font-size: 12px;\">";
            for ( var key in item.splitfiles) {
                repData += "<tr><td>"
                + item.splitfiles[key].name + "</td><td class='badge badge-" + status2cssClass[item.splitfiles[key].status]  + "'>" + item.splitfiles[key].status + "</td>" + "</td></tr>";
            }
            repData += "</table></div>";            
        }        
    } else {
    ...
    .append("<tr class='table-"
            ...
            + status2cssClass[item.status] + "'>" + item.status
            + "</span>" + "</td>" + "<td class='align-middle'>"
            + actions + "</td>" + "</tr>");
}
