...
function replicateFile(handle, fileToReplicate, userChecksum, db, config,
    ...
    logger.debug("Path [%s], checksum: [%s], fileSize: [%d]", resolvedPath, checksum, filesize);
            ...
            if (error) {
                ...
                if (item) {
                    if (item.status === 'QUEUED' || item.status === 'IN PROGRESS') {
                        ...
                        logger.debug("Item status [%s] set to re-queued", item.status);
...
exports.remove = function (req, res, db, config) {
    ...
    logger.debug("/remove [%s] [%s]", handle, filename);
    ...
    db.collection("item").find({
        ...
        if (error) {
            ...
            if (items.length === 1) {
                ...
                logger.warn("Cannot remove the whole directory - remove single files first");
            }
...
exports.replicate = function (req, res, db, config) {
    ...
    logger.debug( "/replicate [%s] [%s]", handle, toReplicate);
    ...
    if (handle && toReplicate) {
        ...
        if (stats.isFile()) {
            replicateFile(handle, toReplicate, userChecksum, db, config, function (response, error) {
                res.send(response);
            });
        ...
        logger.warn( "Missing arguments to /replicate");
        res.send({
            response: "ERROR"
        });
    }
